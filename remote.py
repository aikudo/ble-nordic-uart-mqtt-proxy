...
class UartRxChrc(localGATT.Characteristic):
    ...
    def update(self, value):
        if self.props[constants.GATT_CHRC_IFACE]['Notifying']:
            print('notifying [%s]' % value.strip())
            self.PropertiesChanged(constants.GATT_CHRC_IFACE, 
                { 'Value': encode(value) }, [])
            self.value = ''
        else:
            print('value [%s]' % value.strip())
            self.value = value

    def ReadValue(self, options):
        print('read [%s]' % self.value.strip())
        value = self.value
        self.value = ''
        return encode(value)
...
class UartTxChrc(localGATT.Characteristic):
    ...
    def task(self):
        ...
        if context and context.poll() is None:
            for buf in context.stdout:
                value = buf.decode()
                self.chrc.update(value)

    def handler(self, value):
        if value == "!B516":
            if self.context and self.context.poll() is None:
                value = '> process running\n'
            else:
                self.context = subprocess.Popen(
                    commands[self.index], shell=True, 
        ...
        # down
            ...
            if self.context and self.context.poll() is None:
                ...
                value = '> nothing running\n'
        # left
        elif value == "!B714":
            if self.index == 0: self.index = len(commands)
            self.index -= 1
            value = '> ' + commands[self.index] + '\n'
        # right
        elif value == "!B813":
            self.index += 1
            if self.index == len(commands): self.index = 0
            value = '> ' + commands[self.index] + '\n'
        else:
            value = None
        return value
